...
UINT8                       *mShimPolicyTable = NULL;
STATIC  UINT32              mShimCurrentTableSize = 0;
  ...
  )
{
  ...
  CurrentEntry = (VARIABLE_POLICY_ENTRY*)mShimPolicyTable;
  while ((UINTN)CurrentEntry < (UINTN)((UINT8*)mShimPolicyTable + mShimCurrentTableSize)) {
    // Check for a match.
...
/**
  ...
  @retval     FALSE   An error has occurred or we cannot prove that the variable is locked.
  ...
  )
{
  ...
  mShimCurrentTableSize = 0;
  mShimPolicyTable = NULL;
  Status = DumpVariablePolicy ((UINT8*)mShimPolicyTable, &mShimCurrentTableSize);
  if (Status != EFI_BUFFER_TOO_SMALL) {
    DEBUG ((DEBUG_ERROR, "%a - Failed to determine policy table size! %r\n", __FUNCTION__, Status));
    return FALSE;
  }
  mShimPolicyTable = AllocatePool (mShimCurrentTableSize);
  if (mShimPolicyTable == NULL) {
    DEBUG ((DEBUG_ERROR, "%a - Failed to allocated space for policy table! 0x%X\n", __FUNCTION__, mShimCurrentTableSize));
    return FALSE;
  }
  Status = DumpVariablePolicy ((UINT8*)mShimPolicyTable, &mShimCurrentTableSize);
  if (EFI_ERROR (Status)) {
  ...
  MatchPolicy = GetBestPolicyMatch( VariableName,
                                    VendorGuid,
                                    &MatchPriority );
  ...
  )
{
  ...
  if (!EFI_ERROR( Status )) {
    ...
    if (Status == EFI_ALREADY_STARTED && IsVariableAlreadyLocked (VariableName, VendorGuid)) {
        Status = EFI_SUCCESS;
    }
  }
  if (EFI_ERROR( Status )) {
